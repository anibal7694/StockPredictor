...
exports.fav_info = async (req, res, next) => {
    var user = await User.find({email: req.body.email});
    var companyList =  user[0]["favourite_companies"];
...
exports.historical_stock_info = function(req, res) {
  let hisData,currData;
let hisDataObj ={};
dataFetch.hisDatafetch().then(
function (hisDataResult){
  for(i=0;i<hisDataResult.length;i++){
    let key = hisDataResult[i].company;
    //console.log(key);

    if (key in hisDataObj){
      hisDataObj[key].push(hisDataResult[i]);
    }
    else{
      hisDataObj[key]= [hisDataResult[i]];
    }
  }


//  console.log("length : "+ hisDataObj.length);
  hisDataObj=JSON.stringify( hisDataObj);

  return dataFetch.cmpDetails();
}
).then(
function (companyDetails){
  res.json({hisData: hisDataObj,companyDetails: JSON.stringify( companyDetails)} );
}
).catch(
function (error){
  console.log(error);
...
exports.user_status = function(req,res){
  User.update({email: req.body.email},  {$set: {active: req.body.status}}, function(err, user) {
      if (err)
          res.send(err);
      res.json(user);
  });
...
exports.delete_user_companies = function (req, res) {
    let deleteFavouriteCompany = req.body.favourite_companies;
